...
select top 10 product_id, sum(quantity*sale_price) from df_orders group by product_id order by sum(quantity*sale_price) desc;


-- Find Top 5 highest selling products in each region
with cte as( select region, product_id, 
sum(quantity*sale_price) as revenue from df_orders
